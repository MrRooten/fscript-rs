...
struct RetWithType<T> {
    value: T,
    ret_type: Option<Arc<FSRSType>>,
}

impl<T> RetWithType<T> {
    pub fn new(value: T, ret_type: Option<Arc<FSRSType>>) -> Self {
        Self { value, ret_type }
    }

    pub fn extent_vec(self) -> RetWithType<Vec<T>> {
        RetWithType {
...
pub struct FnDef {
    ...
    defer_stack: Vec<Vec<BytecodeArg>>,
}
...
pub struct BytecodeContext {
    ...
    pub(crate) defer_stack: Vec<Vec<BytecodeArg>>,
}
...
pub struct BytecodeResult {
    ...
    pub(crate) bytecode_map: HashMap<String, Bytecode>,
    pub(crate) type_info: FSRSTypeInfo,
}
...
impl<'a> Bytecode {
    ...
    fn static_getter_inner(
        name: &str,
        offset: &mut Option<usize>,
        father_type: Option<Arc<FSRSType>>,
    ) -> Option<Arc<FSRSType>> {
        if let Some(t) = father_type.as_ref()
            && let FSRSType::Struct(s) = t.as_ref()
        {
            s.fields.get(name).map(|(inn_offset, t)| {
                *offset = Some(*inn_offset);
                t.clone()
            })
        } else {
            None
    ...
    fn load_getter(
        ...
        father_type: Option<Arc<FSRSType>>,
    ) -> RetWithType<Vec<BytecodeArg>> {
        let mut result = Vec::new();
        let name = getter.get_name();
        let mut offset = None;
        let mut from_type = None;
        if const_map.is_static {
            from_type = Self::static_getter_inner(name, &mut offset, father_type.clone());
        }
        ...
        let mut v =
            Self::load_token_with_map(getter.get_getter(), var_map, const_map, false, false)
                .unwrap();
        result.append(&mut v.value[0]);

        let type_info_full = if const_map.is_static {
            if let Some(f) = from_type.clone() {
                Some(f)
            } else {
                let var_type = const_map
                    .type_info
        ...
        RetWithType::new(result, ret_type)
    }
            ...
            *attr_id_arg = Some((id, name.to_string()));
            ...
            if !context.cur_fn_name.is_empty() && name.eq(context.cur_fn_name.last().unwrap()) {
                ...
                if context.variable_is_defined(name) || context.ref_map_stack.is_empty() {
                    let arg = ArgType::Local(LocalVar::new(id, name.to_string(), false, None));
                    result.push(BytecodeArg {
                        operator: BytecodeOperator::LoadVar,
                        arg: Box::new(arg),
                        arg_n: id as i64,
                    ...
                    let arg = ArgType::Global(name.to_string());
                    result.push(BytecodeArg {
                        operator: BytecodeOperator::Load,
                        arg: Box::new(arg),
                        arg_n: 0,
    ...
    fn load_call(
        ...
        for arg in call.get_args() {
            let mut v = Self::load_token_with_map(arg, var_map, context, false, false).unwrap();
            result.append(&mut v.value[0]);
        }
        ...
        let arg = if is_method_call {
            let method_fn_sig = Self::get_method_sig(call, context, father_type.clone());
            if context.is_static {
            ...
            if context.is_static {
                if let Some(sig) = &call_sig {
                    ret_type = sig.return_type.clone();
                }
            }
        ...
        let op_arg = match is_attr {
            ...
            false => {
                ...
                let arg = if context.variable_is_defined(var.get_name()) {
                    ...
                    let arg = ArgType::Global(var.get_name().to_string());
                    BytecodeArg {
                        operator: BytecodeOperator::Load,
                        arg: Box::new(arg),
                        info: Box::new(FSRByteInfo::new(&context.lines, var.get_meta().clone())),
    ...
    fn load_variable(
        ...
        if context.key_map.contains_key(var.get_name()) {
            return Self::load_keyword_var(var, context);
        }

        if context.is_variable_in_ref_stack(var.get_name()) && !var.is_defined {
            return Self::load_closure_var(var, var_map, is_attr, context);
        }

        Self::load_var_fallback(var, var_map, is_attr, context)
    }
    ...
    fn load_assign_arg(
        ...
        if let Some(ref_map) = context.ref_map_stack.last() {
            let is_defined_in_ref = ref_map.get(var.get_name()).cloned().unwrap_or(false);
            if is_defined_in_ref {
                let arg_id = var_map.last_mut().unwrap().get_var(var.get_name()).unwrap();
    ...
    fn special_variable_trigger(
        ...
        var: &FSRVariable,
        context: &mut BytecodeContext,
    ) -> RetWithType<Option<Vec<BytecodeArg>>> {
        let res = if var.get_name().eq("try") {
            (
    ...
    fn load_struct(
        ...
        for function in struct_stmt.get_block().get_tokens() {
            if let FSRToken::FunctionDef(def) = function {
                ...
                if const_map.is_pre_compile {
                    let fn_type = Arc::new(FSRSType::Fn(v.fn_sig.as_ref().unwrap().clone()));
                    const_map.type_info.types.insert(
                        vec![
                            struct_stmt.get_name().to_string(),
                ...
                // let mut tmp_defer_stack = vec![];
                // std::mem::swap(&mut const_map.defer_stack, &mut tmp_defer_stack);
                // let defer_stack = const_map.defer_stack;

                let defer_stack = std::mem::take(&mut const_map.defer_stack);
    ...
    fn load_expr(
        ...
        if const_map.is_pre_compile {
            return RetWithType::new(Vec::new(), None);
        }
        ...
        if let FSRToken::Expr(sub_expr) = expr.get_left() {
            ...
            return_type = v.ret_type;
            op_code.append(&mut v.value);
        ...
        if let FSRToken::Expr(sub_expr) = expr.get_right() {
            ...
            if expr.get_op().eq(".") {
                ...
                if let Some(op_arg) = v.value {
                    second.extend(op_arg);
                    op_code.append(&mut second);
                    return RetWithType::new(op_code, v.ret_type);
                }
            }
            ...
            if expr.get_op().eq(".") || expr.get_op().eq("::") {
                ...
                if let Some(op_arg) = v.0 {
                    second.extend(op_arg);
                    op_code.append(&mut second);
                    return RetWithType::new(op_code, v.1.clone());
                }
            }
            ...
            if expr.get_op().eq(".") || expr.get_op().eq("::") {
                ...
                return RetWithType::new(op_code, return_type);
            }
        ...
        RetWithType::new(op_code, return_type)
    }
    ...
    fn load_block(
        ...
        for token in block.get_tokens() {
            let lines = Self::load_token_with_map(token, var_map, const_map, false, false).unwrap();
    ...
    fn load_if_def(
        ...
        let mut vs = vec![];
        let mut v = Self::load_token_with_map(test_exp, var_map, const_map, false, false).unwrap();
        let mut test_list = Vec::new();
        let mut t = v.value.remove(0);
            ...
            for e in s.get_elses() {
                ...
                if let Some(t) = test_exp {
                    ...
                    test_list.push(BytecodeArg {
                        ...
                        arg: Box::new(ArgType::IfTestNext(skip_len as u64)),
                        info: Box::new(FSRByteInfo::new(
                            &const_map.lines,
                            if_def.get_meta().clone(),
                        )),
                    ...
                    let mut v =
                        Self::load_token_with_map(t, var_map, const_map, false, false).unwrap();
                    let mut t = v.value.remove(0);
                    test_list.append(&mut t);
                    let skip_len = block_items.len() as u64;
                    test_list.push(BytecodeArg {
    ...
    fn load_for_def(
        ...
        let for_line = block_items.len() as u64 + 3;
        t.push(BytecodeArg {
            operator: BytecodeOperator::LoadForIter,
            arg: Box::new(ArgType::ForLine(for_line)),
            info: Box::new(FSRByteInfo::new(
                &const_map.lines,
                for_def.get_meta().clone(),
            )),
            arg_n: for_line as i64,
        });
        result.push(t);
        ...
        let ret_line_delta = result.len() as i64 - 1;
        let end = vec![BytecodeArg {
            operator: BytecodeOperator::ForBlockEnd,
            arg: Box::new(ArgType::ForEnd(ret_line_delta)),
            info: Box::new(FSRByteInfo::new(
                &const_map.lines,
                for_def.get_meta().clone(),
            )),
            arg_n: ret_line_delta,
        }];
    ...
    fn load_while_def(
        ...
        let skip_len = block_items.len() as u64 + 1;
        test_list.push(BytecodeArg {
            operator: BytecodeOperator::WhileTest,
            arg: Box::new(ArgType::WhileTest(skip_len)),
            info: Box::new(FSRByteInfo::new(
                &const_map.lines,
        ...
        let while_len = len as i64 + 1;
        let end = BytecodeArg {
            operator: BytecodeOperator::WhileBlockEnd,
            arg: Box::new(ArgType::WhileEnd(while_len)),
            info: Box::new(FSRByteInfo::new(
                &const_map.lines,
                while_def.get_meta().clone(),
            )),
    ...
    fn get_var_from_last(var_map: &Vec<VarMap>, name: &str) -> Option<u64> {
        var_map.last().unwrap().get_var(name).cloned()
    }
        ...
        if let FSRToken::Expr(expr) = token {
            ...
            match v.0 {
                AttrIdOrCode::Bytecode(bytecode_args) => {
                    return Ok(RetWithType::new(vec![bytecode_args], v.1));
                }
            ...
            return Ok(RetWithType::new(vec![v.0], v.1));
            ...
            if fn_def.is_lambda() {
                ...
                let c_id = Self::get_var_from_last(var_map, fn_def.get_name()).unwrap();
                ...
                return Ok(RetWithType::new(vec![result], None));
            }
            if byte_context.is_pre_compile {
                return Ok(RetWithType::new(vec![], None));
            }
            let v = Self::load_function(fn_def, var_map, byte_context);
            return Ok(RetWithType::new(v.code, None));
        ...
        if const_map.is_static {
            ...
            if let Some(f_type) = father_type {
                if let FSRSType::Struct(father_struct) = f_type.as_ref() {
                    let attr_type = father_struct.fields.get(attr_name).ok_or_else(|| {
                        anyhow!(
                    ...
                    if let FSRSType::Struct(father_struct) = ptr.as_ref() {
                        ...
                        return Err(anyhow!(
                            "Static attribute assignment father must be struct type"
                        ));
                    }
                } else {
                    return Err(anyhow!(
                ...
                return Err(anyhow!("Static attribute assignment must have father type"));
            }
        };
        Ok(())
    }
    ...
    fn load_dot_assign(
        ...
        if let FSRToken::Expr(v) = &**token.get_left() {
            ...
            let attr_id = ensure_attr_id!(var_map, attr_name);
            ...
            result_list.append(&mut right.value[0]);
            result_list.append(&mut left.value[0]);
            ...
            Self::is_static_type_process(&left.ret_type, attr_name, &mut attr_var, const_map);
    ...
    fn load_getter_assign(
        ...
        if let FSRToken::Getter(v) = &**token.get_left() {
            ...
            result_list.append(&mut right.value[0]);
            result_list.append(&mut left.value);
    ...
    fn load_list(
        ...
        for sub_t in token.get_items().iter().rev() {
            let v = Bytecode::load_token_with_map(sub_t, var_map, const_map, false, false).unwrap();
            if const_map.is_static {
                if let Some(t) = &last_type {
                    let is_same = Self::is_same_two_type(&last_type, &v.ret_type);
                    if !is_same {
                        panic!(
                    ...
                    if v.ret_type.is_none() {
                        panic!("Static list item must have type");
                    }
                    last_type = v.ret_type.clone();
                }
            }
            let mut expr = v.value;
            if !expr.is_empty() {
    ...
    fn load_ret(
        ...
        let v = Self::load_token_with_map(ret.get_return_expr(), var_map, const_map, false, false)
            .unwrap();
        Self::check_ret_type(&v.ret_type, const_map);
        let mut ret_expr = Vec::new();
        let mut r = v;
        if !r.value.is_empty() {
            ret_expr.append(&mut r.value[0]);
        }
        ...
        RetWithType::new(ret_expr, r.ret_type)
    }
    ...
    fn load_defer(
        ...
        //let mut result = Vec::new();
        let mut v =
            Self::load_token_with_map(defer.get_defer_expr(), var_map, const_map, false, false)?;
        let defer_expr = if !v.value.is_empty() {
            let r = v.value.remove(0);
            r
        } else {
            panic!("Defer expression cannot be empty");
        };

        const_map.defer_stack.push(defer_expr);
    ...
    fn set_arg_type(v: &FSRVariable, bytecontext: &mut BytecodeContext, call_sig: &mut FnCallSig) {
        let Some(type_hint) = v.get_type_hint() else {
            panic!(
    ...
    fn args_process(
        ...
        load_args: &mut Vec<BytecodeArg>,
        call_sig: &mut FnCallSig,
    ) {
        for arg in args {
            if let FSRToken::Variable(v) = arg {
                ...
                if bytecontext.is_static {
                    Self::set_arg_type(v, bytecontext, call_sig);
                }
            } else {
    ...
    fn should_store_to_cell(name: &str, ctx: &BytecodeContext) -> bool {
        if let Some(ref_map) = ctx.ref_map_stack.last() {
            ref_map.get(name).copied().unwrap_or(false) && ctx.is_variable_in_ref_stack(name)
        } else {
            false
        }
    }

    fn build_ref_map_for_fn(fn_def: &FSRFnDef) -> HashMap<String, bool> {
        let mut map = HashMap::new();
        for (name, info) in fn_def.ref_map.borrow().iter() {
            map.insert(name.to_string(), info.is_defined);
        }
        map
    }

    fn collect_arg_names(args: &Vec<FSRToken>) -> Vec<String> {
    ...
    fn ret_ensure(
        fn_body: &mut Vec<Vec<BytecodeArg>>,
        fn_def: &FSRFnDef,
        bytecontext: &BytecodeContext,
    ) {
    ...
    fn call_type_hint_set(
        call_sig: &mut FnCallSig,
        ret_type: Option<&FSRTypeName>,
        bytecontext: &mut BytecodeContext,
    ) {
    ...
    fn clone_var_map(block_var_map: &VarMap) -> VarMap {
        let mut var_map = VarMap::new("_");
        var_map.attr_id = AtomicU64::new(block_var_map.attr_id.load(Ordering::Relaxed));
        var_map.var_id = AtomicU64::new(block_var_map.var_id.load(Ordering::Relaxed));
        var_map.var_map = block_var_map.var_map.clone();
        var_map.attr_map = block_var_map.attr_map.clone();
        var_map.const_map = block_var_map.const_map.clone();

        var_map
    }

    fn set_fn_call_sig(
        mut call_sig: FnCallSig,
        ret_type: Option<&FSRTypeName>,
        bytecontext: &mut BytecodeContext,
    ) -> Option<Arc<FnCallSig>> {
        let mut call_sig_maybe = None;
        if bytecontext.is_static {
            Self::call_type_hint_set(&mut call_sig, ret_type, bytecontext);
    ...
    fn load_function(
        ...
        let store_to_cell = Self::should_store_to_cell(name, bytecontext);

        let fn_var_map = VarMap::new(fn_def.get_name());
        var_map.push(fn_var_map);

        let hash_map_ref_map = Self::build_ref_map_for_fn(fn_def);
        ...
        Self::args_process(args, var_map, bytecontext, &mut load_args, &mut call_sig);

        let args_save = Self::collect_arg_names(args);

        let body = fn_def.get_body();
        bytecontext.cur_fn_name.push(name.to_string());
        let ret_type = fn_def.ret_type.as_ref();

        let call_sig_maybe = Self::set_fn_call_sig(call_sig, ret_type, bytecontext);

        let cur_name = bytecontext.cur_fn_name.join("::").to_string();

        let mut fn_body = Self::load_block(body, var_map, bytecontext);

        bytecontext.cur_fn_name.pop();

        let block_var_map = var_map.pop().unwrap();

        for sub_def in block_var_map.sub_fn_def.iter() {
        ...
        Self::ret_ensure(&mut fn_body, fn_def, bytecontext);

        bytecontext.def_fn_ret.pop();
        bytecontext.is_static = origin_is_static;

        let var_map = Self::clone_var_map(&block_var_map);

        let defer_stack = std::mem::take(&mut bytecontext.defer_stack);
        let fn_def = FnDef {
            code: fn_body.clone(),
            var_map,
        ...
        FnDesc {
            code: vec![define_fn],
            var_map: block_var_map,
            fn_sig: call_sig_maybe,
        }
    }
    ...
    fn load_isolate_block(
        ...
        let mut v =
            Self::load_token_with_map(token, &mut var_map, const_map, false, false).unwrap();
        let var = var_map.pop().unwrap();
        for sub_def in var.sub_fn_def.iter() {
            v.value.splice(0..0, sub_def.bytecode.clone());
        }

        (v.value, var)
    }

    fn pre_load_ast(_name: &str, token: &FSRToken, lines: Vec<usize>) -> Result<BytecodeContext> {
    ...
    fn load_main_ast(
        name: &str,
        token: FSRToken,
        vs: (Vec<Vec<BytecodeArg>>, VarMap),
    ) -> Result<HashMap<String, Bytecode>> {
        let mut result = vec![];
        for v in vs.0 {
            let single_line = Vec::from_iter(v);
            result.push(single_line);
    ...
    fn load_sub_fn_ast(
        const_table: BytecodeContext,
        res: &mut HashMap<String, Bytecode>,
    ) -> FSRSTypeInfo {
        ...
        for code in codes {
            let bytecode = Bytecode {
                ...
                defer_stack: code.1.defer_stack,
            };

            res.insert(code.0.to_string(), bytecode);
        }

        const_table.type_info
    }

    pub fn load_ast(name: &str, token: FSRToken, lines: Vec<usize>) -> Result<BytecodeResult> {
        let type_info = Self::pre_load_ast(name, &token, lines.clone())?;
        let type_info = type_info.type_info;
        let mut const_table = BytecodeContext::new(lines);
        const_table.type_info = type_info;
        let vs = Self::load_isolate_block(&token, &mut const_table);
        let mut res = Self::load_main_ast(name, token, vs)?;
        let type_info = Self::load_sub_fn_ast(const_table, &mut res);
    ...
    pub fn compile(name: &str, code: &str) -> Result<BytecodeResult> {
