...
use crate::{
    backend::{
        compiler::{
            bytecode::{
                ArgType, BytecodeArg, BytecodeOperator, CompareOperator, FSRDbgFlag, FSRSType,
                FastAttr, FnArgs, FnCallSig, LocalVar, OpAssign,
            },
...
impl IndexMap {
    ...
    pub fn get(&self, i: &u64) -> Option<NonZeroUsize> {
        ...
        //     None => None,
        // }
        self.vs.get(*i as usize).copied().flatten()
    }
...
pub fn index_map_obj_to_ptr(map: &IndexMapObj) -> IndexMapObjPtr {
    let ptr = map as *const IndexMapObj;
    ptr as usize
}

pub fn index_map_obj_from_ptr(ptr: IndexMapObjPtr) -> &'static IndexMapObj {
    let ptr = ptr as *const IndexMapObj;
    unsafe { &*ptr }
}

pub type IndexMapObjPtr = usize;
...
pub struct CallFrame {
    ...
    pub(crate) const_map: Option<IndexMapObjPtr>,
...
impl CallFrame {
    ...
    #[inline]
    fn get_slot(&self, id: usize) -> Option<&BytecodeArg> {
        if let Some(slot) = &self.bytecode_slot {
            let slot = unsafe { &*slot.load(Ordering::Relaxed) };
            return slot.get(id);
        }
        None
    }
    ...
    pub fn get_const(&self, id: &u64) -> Option<&ObjId> {
        index_map_obj_from_ptr(self.const_map.unwrap()).get(id)
    }
    ...
    pub fn new(code: ObjId, fn_obj: ObjId) -> Self {
        Self {
            local_var: Box::new(IndexMap::new()),
...
impl RetState {
    ...
    #[inline]
    pub fn is_break_cur_line(&self) -> bool {
        matches!(self, RetState::BreakCurLine)
    }
...
impl<'a> FSRThreadRuntime<'a> {
    ...
    pub fn process_callframe(work_list: &mut Vec<ObjId>, it: &CallFrame) {
        ...
        if let Some(handling_exception) = it.handling_exception {
            work_list.push(handling_exception);
        }
        ...
        for val in index_map_obj_from_ptr(it.const_map.unwrap()).iter() {
            work_list.push(*val);
        }
        ...
        for obj in index_map_obj_from_ptr(it.const_map.unwrap()).iter() {
            work_list.push(*obj);
        }
    ...
    pub fn thread_unwrap(thread_rt: &FSRThreadRuntime, message: &str) -> Result<(), FSRError> {
    ...
    fn getter_assign_process(
        ...
        let set_item = FSRObject::id_to_obj(container_obj)
            .get_cls_offset_attr(FastAttr::SetItem)
            .unwrap()
    ...
    fn binary_reminder_process(
        ...
        let len = args.len();
        let res = if let Some(rust_fn) = obj_cls!(left_id).get_rust_fn(FastAttr::Reminder) {
            let res = rust_fn(args.as_ptr(), len, self)?;

            res
        } else {
            let res =
                FSRObject::invoke_offset_method(FastAttr::Reminder, &[left_id, right_id], self)?;
    ...
    fn jit_call(
        ...
        let frame =
            self.frame_free_list
                .new_frame(FSRObject::id_to_obj(fn_id).as_fn().code, fn_id, 2);
        self.push_frame(
            frame,
    ...
    fn async_call(&mut self, fn_id: ObjId, args: &mut CallArgs) -> Result<RetState, FSRError> {
        let frame =
            self.frame_free_list
                .new_frame(FSRObject::id_to_obj(fn_id).as_fn().code, fn_id, 2);
        ...
        if fn_obj.is_fsr_function() {
            ...
            if let FSRnE::FSRFn(f) = &fn_obj.as_fn().fn_def {
                if f.jit_code.is_some() {
                    return self.jit_call(fn_id, args, f, ret_type);
                }
    ...
    fn try_end(self: &mut FSRThreadRuntime<'a>) -> Result<RetState, FSRError> {
        let end = self.get_cur_mut_frame().catch_ends.pop().unwrap();
        self.get_cur_mut_frame().ip = (end.1, 0);
        Ok(RetState::BreakCurLine)
    }
    ...
    fn if_test_process(
        ...
        if test_val == FSRObject::false_id() || test_val == FSRObject::none_id() {
            ...
            return Ok(RetState::BreakCurLine);
        }
    ...
    fn else_if_test(
        ...
        if test_val == FSRObject::false_id() || test_val == FSRObject::none_id() {
            ...
            self.get_cur_mut_frame().ip = (tmp + n as usize + 1_usize, 0);
    ...
    fn else_process(
        ...
        if self.get_cur_mut_frame().flow_tracker.peek_last_if_test() {
            let n = bytecode.arg_n;
            self.get_cur_mut_frame().ip = (self.get_cur_frame().ip.0 + n as usize + 1_usize, 0);
            return Ok(RetState::BreakCurLine);
        }
    ...
    // save will fix
    fn for_block_ref(self: &mut FSRThreadRuntime<'a>) -> Result<RetState, FSRError> {
    ...
    fn load_for_iter(
        ...
        let iter_offset = FastAttr::Iterator;
        let read_iter_id = match iter_obj.get_cls_offset_attr(iter_offset) {
            Some(s) => {
    ...
    fn pre_while(&mut self, n: u64) {
        let ip_0 = self.get_cur_frame().ip.0;
        let tracker = &mut self.get_cur_mut_frame().flow_tracker;
        if let Some(s) = tracker.break_line.last().cloned() {
            if ip_0 + n as usize + 1 != s {
                tracker.break_line.push(ip_0 + n as usize + 1);
            }
        } else {
            tracker.break_line.push(ip_0 + n as usize + 1);
        }

        if let Some(s) = tracker.loop_start_line.last().cloned() {
            if ip_0 != s {
                // let ip_0 = self.get_cur_frame().ip.0;
                tracker.loop_start_line.push(ip_0);
            }
        } else {
            // let ip_0 = self.get_cur_frame().ip.0;
            tracker.loop_start_line.push(ip_0);
    ...
    fn define_fn_value(
        self: &mut FSRThreadRuntime<'a>,
        fn_args: &FnArgs,
    ) -> Result<(FSRValue<'a>, bool), FSRError> {
    ...
    fn define_fn_scope(
        ...
        if let Some(cur_cls) = &mut frame.cur_cls {
            let offset = FastAttr::from_alias_name(fn_args.name.as_str());
            ...
            let define_fn_id = self.get_cur_frame().fn_id;

            self.module_fn_define(&fn_args.name, fn_id, is_jit, define_fn_id);

            self.fn_store_to_cell(fn_args, fn_id, define_fn_id);
    ...
    fn define_fn(
        ...
        let (fn_obj, is_jit) = self.define_fn_value(fn_args)?;
        ...
        self.define_fn_scope(fn_args, fn_id, is_jit);
    ...
    fn op_assign_helper(
        ...
        let v = FSRObject::invoke_offset_method(offset, &[left_id, right_id], thread)?.get_id();
        Ok(v)
    }
    ...
    pub fn compare_equal_process(
        ...
        let args = [left, right];
        let len = args.len();
        let v = if let Some(rust_fn) = obj_cls!(left).get_rust_fn(FastAttr::Equal) {
            let res = rust_fn(args.as_ptr(), len, self)?;

            res.get_id() == FSRObject::true_id()
        } else {
            let v = FSRObject::invoke_offset_method(FastAttr::Equal, &[left, right], self)?
                .get_id()
                == FSRObject::true_id();
    ...
    fn raise_process(self: &mut FSRThreadRuntime<'a>) -> Result<RetState, FSRError> {
        let exception_id = pop_exp!(self).unwrap_or(FSRObject::none_id());
    ...
    #[cfg_attr(feature = "more_inline", inline(always))]
    fn try_exception_process(self: &mut FSRThreadRuntime<'a>) -> Result<RetState, FSRError> {
    ...
    fn end_fn(self: &mut FSRThreadRuntime<'a>) -> Result<RetState, FSRError> {
        self.pop_stack();
        let cur = self.get_cur_mut_frame();
        //let code = cur.code;
    ...
    fn for_block_end(
        ...
        let tmp = self.get_cur_frame().ip.0;
        self.get_cur_mut_frame().ip = (tmp - bytecode.arg_n as usize, 0);
        return Ok(RetState::BreakCurLine);
    }
    ...
    fn while_block_end(
        ...
        let ip_0 = self.get_cur_frame().ip.0;
        self.get_cur_mut_frame().ip = (ip_0 - bytecode.arg_n as usize, 0);
        return Ok(RetState::BreakCurLine);
        //}
    }
    ...
    fn assign_closure(
        ...
        let fn_obj = FSRObject::id_to_mut_obj(fn_obj_id)
            .ok_or_else(|| FSRError::new("Not a function object", FSRErrCode::NoSuchObject))?
            .as_mut_fn();
    ...
    fn obj_default_method(self: &mut FSRThreadRuntime<'a>, new_cls: &mut FSRClass) {
        let default_equal = FSRFn::from_rust_fn_static(
            crate::backend::types::class::class_default_equal,
            "object_equal",
        );
        new_cls.insert_offset_attr(FastAttr::Equal, default_equal);
        let default_not_equal = FSRFn::from_rust_fn_static(
            crate::backend::types::class::class_default_not_equal,
            "object_not_equal",
        );
        new_cls.insert_offset_attr(FastAttr::NotEqual, default_not_equal);
    }
    ...
    fn class_def(
        ...
        let ArgType::Local(var) = bytecode.get_arg() else {
            return Err(FSRError::new(
                "not a class def local",
                FSRErrCode::NotValidArgs,
            ));
        };
        let mut new_cls = FSRClass::new(&var.name);
        Self::obj_default_method(self, &mut new_cls);
    ...
    fn post_loop(self: &mut FSRThreadRuntime<'a>) {
        let tracker = &mut self.get_cur_mut_frame().flow_tracker;
    ...
    #[inline]
    fn last_for_iter_obj(&self) -> ObjId {
        self.get_cur_frame()
            .flow_tracker
            .for_iter_obj
            .last()
    ...
    fn special_load_for(
        ...
        let obj = self.last_for_iter_obj();
        ...
        let state = self.get_cur_mut_frame();
        state.insert_var(arg.arg_n as u64, res_id);
        return Ok(RetState::Normal);
    }
    ...
    fn not_process(self: &mut FSRThreadRuntime<'a>) -> Result<RetState, FSRError> {
    ...
    #[cfg_attr(feature = "more_inline", inline(always))]
    fn empty_process() -> Result<RetState, FSRError> {
        Ok(RetState::Normal)
    }

    fn is_catchable(&self) -> bool {
        for frame in self.call_frames.iter().rev() {
            if !frame.catch_ends.is_empty() {
                return true;
            }
        }
    ...
    fn exception_unwrap(&mut self, e: FSRError, bytecode: &BytecodeArg) -> Result<(), FSRError> {
        ...
        let s = if let FSRErrCode::RuntimeError(rt) = e.inner.code {
            let s = FSRObject::id_to_obj(rt).to_string(self);
            let s = if let FSRValue::String(s) = &s {
                s.to_string()
            } else {
    ...
    fn match_process(
        ...
        let v = match op {
            ...
            BytecodeOperator::WhileTest => Self::while_pre_process(self, bytecode),
            ...
            BytecodeOperator::LoadVar => {
                // Self::load_local_var(self, bytecode)
                if let Some(id) = self.get_cur_frame().get_var(&(bytecode.arg_n as u64)) {
                    push_exp!(self, id.get());
                    Ok(RetState::Normal)
                } else {
                    // panic!("variable not found: {}, location: {:?}", bytecode.arg_id, bytecode.get_pos());
                    Self::load_var(self, bytecode)
                }
            }
            ...
            BytecodeOperator::Raise => Self::raise_process(self),
            _ => {
    ...
    #[cfg_attr(feature = "more_inline", inline(always))]
    fn process(&mut self, bytecode: &BytecodeArg) -> Result<RetState, FSRError> {
        let v = self.match_process(bytecode);
    ...
    fn get_chains(thread: &FSRThreadRuntime, state: &CallFrame, var: &LocalVar) -> Option<ObjId> {
        ...
        if !is_base_fn!(fn_id) {
            ...
            if let Some(s) = obj.get_closure_var(var.name.as_str()) {
                return Some(s);
            }
        }
    ...
    fn load_const(&mut self, arg: &BytecodeArg) -> Result<RetState, FSRError> {
        if let Some(id) = self.get_cur_frame().get_const(&(arg.arg_n as u64)).cloned() {
            ...
            Err(FSRError::new(
                format!("Constant at index {} not found", arg.arg_n),
                FSRErrCode::NoSuchObject,
            ))
        }
    }
    ...
    //#[cfg_attr(feature = "more_inline", inline(always))]
    fn load_var(&mut self, arg: &BytecodeArg) -> Result<RetState, FSRError> {
        ...
        match arg.get_arg() {
            ...
            ArgType::Const(index, _) => {
                let obj = *self.get_cur_frame().get_const(index).unwrap();
                push_exp!(self, obj);
            }

            ArgType::ClosureVar(v) => {
                let var = self.fetch_closure_variable(v)?;
    ...
    fn err_to_err_obj(&mut self, err: &FSRError) -> ObjId {
        let err_string = format!("{:?}: {}", err.inner.code, err.inner.msg);

        let s = FSRString::new_value(err_string);
        self.garbage_collect
            .new_object(s, GlobalObj::StringCls.get_id())
    }

    fn exception_process(&mut self, err: &FSRError) -> bool {
        // if let Some(exception_obj) = self.get_cur_frame().handling_exception {
        //let exception_obj = self.err_to_err_obj(err);
        let exception_obj = if let FSRErrCode::RuntimeError(raise_id) = err.inner.code {
            raise_id
        } else {
            self.err_to_err_obj(err)
        };
        self.get_cur_mut_frame().handling_exception = Some(exception_obj);
    ...
    #[cfg_attr(feature = "more_inline", inline(always))]
    fn run_expr_wrapper(&mut self, expr: &Vec<BytecodeArg>) -> Result<bool, FSRError> {
        // if expr.is_empty() {
    ...
    #[inline]
    fn cache_slot(&mut self, expr: &Vec<BytecodeArg>) {
        self.get_cur_mut_frame().bytecode_slot = Some(AtomicPtr::new(
            expr as *const Vec<BytecodeArg> as *mut Vec<BytecodeArg>,
        ));
    }

    #[cfg_attr(feature = "more_inline", inline(always))]
    fn run_expr(&mut self, expr: &Vec<BytecodeArg>) -> Result<bool, FSRError> {
        let mut ret;
        //self.pre_expr(expr);
        ...
        while let Some(arg) = expr.get(self.get_cur_frame().ip.1) {
            ...
            self.counter += 1;
            ret = self.process(arg);
            #[cfg(feature = "bytecode_trace")]
            self.after_bytecode_trace();

            if ret.is_err() {
                if !self.get_cur_frame().catch_ends.is_empty()
                    && Self::exception_process(self, &ret.as_ref().err().unwrap())
                {
                    clear_exp!(self);
                    clear_middle_exp!(self);
                    return Ok(false);
    ...
    pub fn load(&mut self, main_fn: ObjId) -> Result<ObjId, FSRError> {
        ...
        let frame = self.frame_free_list.new_frame(code_id, 0, 2);
        let const_map = Self::get_const_map(self, code.as_code())?;
        self.push_frame(frame, index_map_obj_to_ptr(&const_map));
        //self.unlock_and_lock();
        //self.get_cur_mut_frame().fn_id = 0;
        self.get_cur_mut_frame().is_module = true;
        let mut code = &FSRObject::id_to_obj(code_id)
            .as_code()
            .get_bytecode()
            .bytecode;
    ...
    fn compile_jit_fn(&self, module: ObjId) {
    ...
    pub fn start(&mut self, module: ObjId, start_dbg: bool) -> Result<(), FSRError> {
        ...
        self.cur_frame.const_map = Some(index_map_obj_to_ptr(&const_map));
        ...
        Self::compile_jit_fn(self, module);
