...
use crate::ast::{SyntaxErrType, SyntaxError, token::ASTContext};
...
impl BracketState {
    ...
    pub fn is_string(&self) -> bool {
        self == &Self::SingleQuote || self == &Self::DoubleQuote
    }
}
...
impl ASTParser {
    ...
    pub fn helper(
        ...
        if c == '\'' && (!states.is_empty() && states.peek().0 == BracketState::SingleQuote) {
            states.pop_state();
            return Ok(());
        }

        if c == '\'' {
            states.push_state(BracketState::SingleQuote, offset);
            return Ok(());
        }

        if c == '"' && (!states.is_empty() && states.peek().0.is_bracket()) {
            states.push_state(BracketState::DoubleQuote, offset);
            return Ok(());
        }

        if c == '"' && (!states.is_empty() && states.peek().0 == BracketState::DoubleQuote) {
            states.pop_state();
            return Ok(());
        }
