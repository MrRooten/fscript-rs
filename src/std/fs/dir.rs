...
use crate::{backend::{types::{any::{ExtensionTrait}, base::{FSRObject, FSRRetValue, FSRValue, GlobalObj, ObjId}, class::FSRClass, fn_def::FSRFn, list::FSRList, string::FSRString}, vm::thread::FSRThreadRuntime}, utils::error::FSRError};
use std::fmt::Debug;
...
pub fn fsr_fn_sub_path(
    ...
    if let FSRValue::String(s) = &path_obj.value {
        ...
        for entry in fs::read_dir(dir_path).map_err(|e| FSRError::new(format!("Read dir error: {}", e), crate::utils::error::FSRErrCode::RuntimeError))? {
            let entry = entry.map_err(|e| FSRError::new(format!("DirEntry error: {}", e), crate::utils::error::FSRErrCode::RuntimeError))?;
            let sub_path = entry.path().file_name().unwrap().to_string_lossy().to_string();
            sub_paths.push(sub_path);
        }

        let path_vec = sub_paths.iter().map(|x| {
            let value = FSRString::new_value(x);
            thread.garbage_collect.new_object(value, GlobalObj::StringCls.get_id())
        }).collect::<Vec<_>>();
        let value = FSRList::new_value(path_vec);
        let res = thread.garbage_collect.new_object(value, GlobalObj::ListCls.get_id());
