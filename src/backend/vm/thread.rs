...
pub enum SValue<'a> {
    ...
    Reference(&'a AtomicObjId, bool), // call_method
}
...
impl<'a> SValue<'a> {
    ...
    pub fn get_global_id(&self, thread: &FSRThreadRuntime) -> Result<ObjId, FSRError> {
        Ok(match self {
            ...
            SValue::Reference(atomic_usize, _) => atomic_usize.load(Ordering::Relaxed),
        })
    }
...
impl<'a> FSRThreadRuntime<'a> {
    pub fn stop(&mut self, pair: Arc<(Mutex<bool>, Condvar)>) {
        println!("stop thread");
        self.stop = Some(pair);
    }
    ...
    fn assign_process(
        ...
        let to_assign_obj_svalue = context.exp.pop().unwrap();
        ...
        match assign_id {
            ...
            SValue::Reference(atomic_usize, _) => {
    ...
    fn binary_sub_process(
        ...
        let v2_id = v2.get_global_id(self)?;
        let res =
            FSRObject::invoke_binary_method(BinaryOffset::Sub, v2_id, v1_id, self, context.code)?;
        v1.drop_box(&mut self.thread_allocator);
        v2.drop_box(&mut self.thread_allocator);
        match res {
            FSRRetValue::Value(object) => {
    ...
    fn binary_mul_process(
        ...
        let res =
            FSRObject::invoke_binary_method(BinaryOffset::Mul, v2_id, v1_id, self, context.code)?;
    ...
    fn binary_dot_process(
        ...
        let attr_id = match context.exp.pop().unwrap() {
            ...
            SValue::Reference(atomic_usize, _) => todo!(),
        };
        let dot_father_svalue = match context.exp.pop() {
            Some(s) => s,
            None => {
                return Err(FSRError::new(
        ...
        if dot_father_obj.is_code() {
            ...
            context.exp.push(SValue::Reference(id, false));
        ...
        if let Some(id) = id {
            ...
            context
                .exp
                .push(SValue::Attr(self.thread_allocator.new_box_attr(
                    attr_id.attr_id,
                    dot_father,
                    Some(id),
                    name,
    ...
    fn call_process(
        ...
        let fn_id = match context.exp.pop().unwrap() {
            ...
            SValue::BoxObject(_) => todo!(),
            SValue::Reference(atomic_usize, is_object) => {
        ...
        if fn_obj.is_fsr_cls() {
            ...
            if call_method {
                let self_obj = context.exp.pop().unwrap().get_global_id(self).unwrap();
        ...
        if let ArgType::DefineFnArgs(n, arg_len) = bytecode.get_arg() {
            ...
            if name.2 {
                ...
                if let Some(s) = define_fn_obj.store_cells.get(name.1.as_str()) {
                    let origin_var = s.load(Ordering::Relaxed);
    ...
    fn load_var(
        ...
        if let ArgType::Variable(var) = arg.get_arg() {
            ...
            exp_stack.push(SValue::Attr(
                self.thread_allocator
    ...
    #[inline(always)]
    fn exception_process(&mut self, context: &mut ThreadContext<'a>) -> bool {
